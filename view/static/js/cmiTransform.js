function toggleExtraFields() {
    ...
    const extraFieldsVYC = document.getElementById('extrafields-v_y_c');
    const extraFieldsGastos = document.getElementById('extrafields-gastos');
    
    // Ocultar ambos conjuntos de campos adicionales
    extraFieldsVYC.style.display = 'none';
    extraFieldsGastos.style.display = 'none';
    
    // Mostrar los campos adicionales según la categoría seleccionada
    if (category === 'v_y_c') {
        extraFieldsVYC.style.display = 'block';
    } else if (category === 'gastos') {
        extraFieldsGastos.style.display = 'block';
    } 
}
...
async function executeETL(category, fecha) {
    if(category == 'v_y_c') {
        ...
        try {
            ...
            const response = await fetch('http://172.15.1.161:8510/cmi/api/process_cmi/v_y_c', {
                method: 'POST',
                headers: {
        ...
        try {
            ...
            const response = await fetch('http://172.15.1.161:8510/cmi/api/process_cmi/gastos', {
                method: 'POST',
                headers: {
                'Content-Type': 'application/json',
            },
