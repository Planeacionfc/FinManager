...
def sort_data(df_axis: pd.DataFrame, df_cell: pd.DataFrame) -> None:
    if isinstance(df_axis, pd.DataFrame) & isinstance(df_cell, pd.DataFrame):
        ...
        df_final = df_final[
            ...
            (df_final['CONCEPTO'] == 'Provisión de Inventario') &
        ...
        return hoja_provision(df_final)
    else:
        return 'BAD'

def hoja_provision(df_prov: pd.DataFrame):
    try:
        # Mapear y reemplazar marcas
        mapa_marcas = {
            'BEAUTYHOLICS': 'OTRAS MP',
            'PREMIOS DE MOTIVACIÓN VENTA DIRECTA': 'OTRAS MP',
            'PREBEL': 'OTRAS MP','DYCLASS': 'OTRAS MP',
        ...
        df_prov['MARCA'] = df_prov['MARCA'].replace(mapa_marcas)
        ...
        df_prov['CANAL'] = df_prov['CANAL'].replace(mapa_canales)
        df_prov.loc[(df_prov['CANAL'] == 'SIN ASIGNAR') & (df_prov['MARCA'] != 'SIN ASIGNAR'), 'CANAL'] = 'OTROS'
        return df_prov
